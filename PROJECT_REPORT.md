...
这次我对调参的理解也变实际了一点。刚开始我会想当然地改参数，比如改 learning rate 或 batch size，然后只看最后的 reward。后来发现这样很难判断到底是哪一个改动起了作用。

所以我现在更倾向于先固定一个 baseline，再一个一个改变量。比如先只换 seed，看结果本身波动有多大；再单独改 learning rate 或 `n_steps`。这样虽然慢一点，但至少能看清楚变化是从哪里来的。
...
这个项目对我来说最大的收获，是我不再只把 RL 理解成“选一个算法然后调参数”。真正跑起来以后，我发现训练脚本、配置、日志、评估方式和模型保存都很重要。

这次的 reward 分数并不算高，我也不想把它包装成已经训练得很好。更准确地说，它是一个 baseline：我把 PPO 在 HalfCheetah 上完整跑通了，也知道下一步该怎么继续改进。

后面如果继续做，我会把重点放在更长训练、多 seed 对比和 SAC/reward shaping 对比上。这样结果会比现在只跑一次 200k steps 更有说服力。
